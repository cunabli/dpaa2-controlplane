...
## 2. Port-tenant sum with the kernel default (design D2)

- [ ] 2.1 (bead dpaa2-controlplane-093.2) `Port.tenant` becomes the
  `PortTenant` sum (`Kernel` default | `Named(TenantName)`) in `intent.rs`
  and the same sum in `types.qnt`; delete `effTenant` from `intent_raw.qnt`
  and the parse eff-mapping (parse normalises omitted and explicit `kernel`
  to the `Kernel` case); compile/derive match on the sum so no refusal can
  ever name `kernel` as absent; drop
  the PASS2-F13 DEVIATION sentence that documented the wrinkle; regenerate
  traces whose ITF carries `tenant:""` ports. Verify: the "port without a
  tenant belongs to the kernel" scenarios green; `rawDuplicateNameTrace`
  replays.
