...
### D2. One shared tenant-reference sum; the port default lives at parse

Every place the vocabulary refers to a tenant that may be the reserved
kernel — `Port.tenant` and both link ends — uses one shared sum:

```rust
pub enum TenantRef {
    Kernel,               // the reserved root tenant
    Named(TenantName),    // a declared tenant
}
```

The name is already the model's own: `intent_raw.qnt` calls this
relationship a tenant ref (`tenantRefRefusals`, `hasDanglingTenantRef`),
and the sum follows the `Member` idiom (`intent.rs:177`) — the vocabulary's
existing enum of typed references to declared constructs.

The sum carries **no `Default` impl**: a tenant reference is never optional
in the API, and a programmatic `Intent` states every reference explicitly.
"Omitted means kernel" is a TOML-boundary rule, so *parse* supplies
`Kernel` when a port names no tenant — defaulting is a feature at the
operator surface and a footgun in the API (a zero-initialized link end
silently becoming a kernel end must stay unrepresentable). Parse is also
the one normalisation point: an explicit `tenant = "kernel"` or a `kernel`
link end becomes the `Kernel` case; `effTenant` dies.

Compile and derive then match on the case everywhere: the D6a kernel
allowance at link ends stops being an `is_kernel()` string exemption
(`refuse.rs:436,441`) and becomes a variant arm, as does derive's kernel
link-end dpni materialisation. `TenantAbsent` — at ports *and* link ends —
can only fire on `Named` with an undeclared name, so the `tenant:"kernel"`
wrinkle is structurally unrepresentable at every site, and the vocabulary
has exactly one encoding of "kernel or a declared tenant" instead of two.

*Alternatives considered*: `Option<TenantName>` for ports — rejected: same
...
branch instead of a match arm. A port-only sum with `#[derive(Default)]`,
link ends keeping `TenantName` + the D6a exemption — rejected on review:
it leaves the `is_kernel()` string-test disease alive at link ends and a
second encoding of the same relationship, and a later unification would
re-open the whole spine this change exists to churn once; dropping the
`Default` (the operator default belongs to parse, not the type) removes
the only obstacle to sharing the sum. Keeping `effTenant` with a referrer
arm marking the synthesized kernel — rejected: compile cannot distinguish
a typed `kernel` from a synthesized one without parse smuggling that bit
anyway. The sum is new on both sides (today the model carries the same
`""`/`effTenant` encoding as the Rust): it is authored in `types.qnt`
first with the same case names the Rust enum takes — `Kernel | Named`.
...
with payload, `Tenant` loses `pool`, the `TenantRef` sum at ports and link
ends), `refuse.qnt`
...
payload, and the shared `TenantRef` sum with its parse-time port default.
No other ADR-0013 section changes;
...
- [Breaking programmatic API: every `Tenant`/`Port` literal in tests and
  ...
  list.
- [D2 touches derive's kernel materialisation path at ports and link ends]
  → behavior is pinned by existing scenarios ("a port without a tenant
  belongs to the kernel", "the kernel is nameable at a link end"); the
  conformance suite (`raw_conformance`) and frozen traces replay must stay
  green, and traces whose ITF carries `tenant:""` ports or string link ends
  are regenerated, not hand-patched.
