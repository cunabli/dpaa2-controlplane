...
- **BREAKING** (programmatic API): `Port.tenant` and both link ends become
  one shared two-case sum, `TenantRef::Kernel | Named(TenantName)` (the
  name the model already uses for this relationship), with no `Default`
  impl — "omitted means kernel" is a TOML rule the parser applies, never
  an API default. The `""`→`kernel` eff-mapping and the link-end
  `is_kernel()` string exemptions die: the kernel path is a match arm at
  every site, and the ownerless-port wrinkle stops rendering a `kernel`
  name the operator never typed (design D2 records the alternatives
  considered).
