...
#### Scenario: A port without an owner belongs to the kernel
...
### Requirement: Companion counts derive from regime and budget, never from the operator
The compiler SHALL derive the thread count T from the declared
rate-class table bounded by `max_cores`, and the companion set from the
regime per ADR-0012: poll-mode per child container dpio = 2·T, dpbp =
...
#### Scenario: Poll-mode router derives its companions
- **WHEN** a poll-mode consumer owns two 10G ports and the table gives
  T = 5
- **THEN** the plan holds one child DPRC, two dpnis with at least 5
  ...
  queue, each keyed (consumer, family, ordinal) with its label rendered
...
`Refusal` and `Regime` types SHALL be `#[non_exhaustive]`, and a
`PoolShortfall` variant SHALL be reserved for the reconciler's
live-census refusal.

#### Scenario: Core budget exceeded
- **WHEN** a poll-mode consumer's table row gives T = 5 and
  `max_cores` = 4
- **THEN** compilation is refused with `CoreBudgetExceeded` naming the
  consumer, T, and the budget

#### Scenario: Two consumers overdraw the buffer-pool ceiling
- **WHEN** three poll-mode consumers need 6 dpbps and the inventory
  lists 5
...
- **WHEN** the plan's dpbp entries for a poll-mode consumer are
  inspected
- **THEN** each reads rule `dpbp-pair`, construct the consumer's name,
  anchor `ADR-0012`

#### Scenario: Provenance reaches the declared construct
- **WHEN** a poll-mode consumer's dpio count is explained
- **THEN** the tree reads `dpio ×10 ← 2·T ← T = 5 ← rate-table(10G×2,
  unmeasured) ← port wan0 rate = 10G, port wan1 rate = 10G`, and each
  node names its anchor

### Requirement: Derived objects are keyed, labels are rendered
Each derived object SHALL be identified by the key (consumer, family,
...
- **WHEN** the plan is emitted for a consumer named `router`
...
dpni connected twice, or a consumer object in the root container cannot
be constructed. Emission order SHALL follow `object-model.md` §5 (pool
companions before consumer objects; a kernel-regime dpio before its
dpmcp) as a property of construction.

#### Scenario: No free-standing companion
- **WHEN** code attempts to add a dpio to a plan without a consumer
- **THEN** it does not compile

#### Scenario: Order is a property of construction
- **WHEN** any plan is emitted
- **THEN** every companion precedes the consumer object that draws it,
...
a core budget; virtual fabric between two containers; userspace router
...
- **THEN** the diff reports the poll-mode child's two extra dpmcps and
  nothing else unexplained
