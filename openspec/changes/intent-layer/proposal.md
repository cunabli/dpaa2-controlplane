...
have to know by default: a poll-mode consumer with T threads needs 2·T
dpios, two dpbps, T transmit queues and one dpmcp per process; a kernel
consumer draws one per CPU and one per object; a dpsw is only kernel-bindable
...
- **The intent vocabulary** — the frontend-neutral intermediate
  representation in `dpaa2-api`: a *consumer* (name, regime
...
- **The derivation compiler** — a pure, total
  ...
  (consumer, family, ordinal) with its label rendered from the key
...
- **The plan's relationships are locked by construction**: containment
  ...
  only as a consumer's derivation, and lifecycle ordering
...
- **Derived floors are requests; overrides are limits** — the
  ...
  per-family override under the consumer is a limit, and limit ≥ request
...
- **The thread-count rule is declared, not measured**: T derives from a
  rate-class table seeded from the reference board (two 10G ports in
  poll-mode ⇒ T = 5); provenance marks it *unmeasured*, and ADR-0005 is
  ...
  own). A consumer's `max_cores` bounds it — a plan that needs more is
...
- **Model first, then a hard gate, then Rust** — `models/intent/`
  ...
  ports under a core budget; a virtual fabric between two containers
...
- **BREAKING**: the port-only `topology.toml` schema is replaced, not
  ...
  next break has a hook; a file with no `[[consumer]]` still parses
  (ports default to the kernel consumer in the root container — today's
  behavior), but the neutral `DesiredTopology` type changes shape and
  nothing preserves the old one. `Refusal` and `Regime` are
  `#[non_exhaustive]`: a live-census shortfall variant and a passthrough
  regime (a VFIO child whose guest regime the host cannot see, #4) are
...
- Housekeeping folded in: `object-model.md` §4 takes ADR-0012's settled
  ...
  re-anchors DPDCEI-I1 to this change's consumer-absence rule; dpdmux
...
- `topology-config`: the on-disk schema speaks constructs (consumer,
  port with rate and owner, link, fabric, crypto) and converts into the
  neutral `Intent`, not a port list; ports without an owner default to
  the kernel consumer; validation covers construct references and the
