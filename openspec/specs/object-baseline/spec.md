# object-baseline Specification

## Purpose
Establish an exhaustive, evidence-anchored baseline of the DPAA2 object
surface — one document per restool object family, a cross-family
relationship map, a traffic-classified validation inventory, a pinned
reference environment, the change-series roadmap, and the scoping-session
ADRs — so the later ports (typestate CLI, Quint/TLA+ models, Rust backends)
build on documented behavior rather than re-reading source.
