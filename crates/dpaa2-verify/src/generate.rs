...
fn preamble(spec: &SuiteSpec, cap_steps: usize) -> String {
    // When the run issues MC commands the kernel gates on CAP_NET_ADMIN,
    // the operator must run the suite as root (docs/baseline/mc-ioctl-policy.md,
    // ADR-0004 §7); the count names how many steps need it.
    let operator = if cap_steps == 0 {
        String::new()
    } else {
        format!(
            "# operator: run as root — {cap_steps} steps issue MC commands the kernel gates on CAP_NET_ADMIN (docs/baseline/mc-ioctl-policy.md)\n"
        )
    };
    format!(
        r#"#!/bin/sh
...
{operator}# Every board-touching command is visible below; the model's expected
