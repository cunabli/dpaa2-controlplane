...
/// info` reports), and the tenant that terminates it (design D1 (restool-baseline); `types.qnt`
...
pub struct Port {
    ...
    /// derivation never reads (design D9; ADR-0006). It rides on the port so
    ...
    /// likewise an actuation-only fact the derivation never reads (design D9; ADR-0006),
