...
| 0x4 | No privilege | dprc · assign `--plugged=1` | Plugging an object inside a container locked with `set-locked --locked=1`; the object reads back unplugged. `set-label` on the same object under the same lock is accepted | MC | V-DPRC-3 rev 1 |
| 0x6 | Configuration error | dprc · create | A child container created under a container made without `DPRC_CFG_OPT_SPAWN_ALLOWED`; the parent stays empty. The option-bit refusal is not No privilege | MC | V-DPRC-2-NOSPAWN-1 rev 1 |
| 0x8 | No resources | dpbp · create | A dpbp created into a container made without `DPRC_CFG_OPT_ALLOC_ALLOWED`: the container cannot draw the id from its parent's pool, so the option-bit refusal surfaces as a resource shortfall | MC | V-DPRC-2-NOALLOC-1 rev 1 |
| 0x8 | No resources | dpbp · create; dpni · create | A resource ceiling met by a create-until-refused walk in a scratch child: a dpbp is refused exactly when the buffer pool's free count reaches zero (63 created, the 64th refused), and a dpni is refused at the 18th with every listed pool still showing room (an unlisted resource, ADR-0011). Read from the hook's RECORD lines, not a scored step | MC | V-CEIL-1 rev 1 |
