...
<!-- This is the shared template for all 16 family documents (openspec change
     restool-baseline, spec `object-baseline`). Every section must be present
     in every family document; a section with nothing to say states that
     explicitly — an empty section is a finding, not a failure. -->
...
candidate: a stable id (`FAMILY-I<n>`), the proposition (state predicate,
