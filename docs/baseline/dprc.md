...
| DPRC-I2 | Plug gating: object bound to a kernel driver ⟺ plugged ∧ matching driver present; `assign --plugged=1` ⇒ eventually bound, `--plugged=0` ⇒ released | plugged column of `dprc show`; presence of `driver` symlink under `/sys/bus/fsl-mc/devices/<obj>/` | candidate |
...
| DPRC-I9 | Teardown reachability (liveness): from every reachable scratch-container state some finite action sequence empties and destroys the container | suite replay ending in `destroy` success + container absent from `list` | board-pending (depends on unknown #1) |
