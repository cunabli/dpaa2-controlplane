...
| DPBP-I2 | Pool-entry precondition: allocatable ∈ kernel pool ⟺ plugged ∧ allocator-bound; unplugged objects are dprc-visible but allocator-invisible | `dprc show` plugged column vs driver symlink vs consumer probe outcome | candidate — child half board-anchored (V-POOL-1 rev 1, 2026-08-29: every plugged resident is MC-listed in the scratch child yet has no bus node); the kernel-pool half needs a plugged, bound child, which restool cannot make (V-POOL-1 rev 2, 2026-08-29: the tool refuses `--plugged` on a dprc) — root-only until a DPL-defined child or the raw command path (#10) |
| DPBP-I3 | **Breaking:** the model must NOT assume pool membership ⇒ clean object state — free does no MC reset; cleanliness is established by the next allocator's reset, or never (non-kernel consumers) | buffer count of a re-allocated dpbp before its consumer resets it | candidate — the free path is Linux-side and not restool-observable (V-POOL-2 rev 1 recorded the dpbp plugged and MC-listed but the unplugged child never bound, so the drain was never driven) |
| DPBP-I4 | Exhaustion liveness: consumer probe blocks (silent defer) exactly while `free_count = 0`; adding + plugging one dpbp unblocks it | dmesg defer loop; probe completes after top-up | verified (C1 class, ADR-0001); the kernel exhaustion→top-up face needs a plugged, bound child, which restool cannot make (V-POOL-1 rev 2, 2026-08-29: the tool refuses `--plugged` on a dprc) — root-only until a DPL-defined child or the raw command path (#10) |
...
| DPBP-I7 | Pool-floor ceiling: a `dpbp create` is refused with No resources (0x8) exactly when the buffer-pool free count reaches zero, and a destroy returns the unit — so the `mc.global --resources` census predicts the refusal to the object | create-until-refused count vs the pool free count; `--resources` before/at-ceiling/after | verified 2026-08-29 (V-CEIL-1 rev 1) |
