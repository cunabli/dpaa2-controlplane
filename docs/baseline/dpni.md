...
| `--mac-entries` / `--mac-filter-entries` | `mac_filter_entries` | 1–80 | 80 | ls-addni (old spelling only) — the pair is mutually exclusive; new spelling exists "to align with the options passed through DPL" |
| `--vlan-entries` / `--vlan-filter-entries` | `vlan_filter_entries` | 1–16 | 0 = VLAN filtering disabled | ls-addni (old spelling); dprc-script (16) |
| `--qos-entries` | `qos_entries` | 1–64 | 64 | ls-addni; dprc-script (64) |
...
| DPNI-I7 | Create-default determinism: an omitted create option ⇒ 0 on the wire ⇒ MC default (1 queue, 1 TC, 80 MAC entries, 64 QoS, 64 FS, VLAN filtering off, one CG/TC) | `dpni info` of a bare `dpni create` | board-pending |
