...
|---|---|---|---|
| DPDCEI-I1 | Consumer-unreachable: the DCE block exists on this SoC (Table 2-1) but no software in the corpus can drive a dpdcei — the model refuses dpdcei intents on consumer absence, not hardware absence | manual Table 2-1; `dpdcei create` probe status (`0x0` expected now; `0xB`/`0x8` would re-open the hardware question) | modeled by the intent-layer's consumer-absence rule (`TenantAbsent`, models/intent/refuse.qnt, #3 delivered); the create face landed (V-LIFE-DPDCEI-1) and the DCE module reads linked at API 2.3 (V-READBACK-1, 2026-08-25); the consumer-absence probes stay open → V-DPDCEI-1 under `tier-c-families` (#13) |
