...
| DPDBG-I4 | Object-family membership: a created dpdbg is visible in `dprc show` and enumerates on the fsl-mc bus driver-less — indistinguishable in listing mechanics from any other family | `dprc show`; /sys/bus/fsl-mc/devices | verified — `dprc show` face 2026-08-24 (V-DPDBG-1 driven trace: create/plug/destroy all read back); sysfs face 2026-09-14 (V-DPDBG-2 rev 1, 8/8, dprc-encapsulation task 6.1): the bus node was present right after create with no sync or rescan issued (root-container autorescan, the V-DPRC-4/V-DPRC-5 route), held no driver link either unplugged or plugged (modalias `fsl-mc:v00001957ddpdbg`, no driver for the type), and was gone in the post-destroy read-back |
