...
## Ioctl command policy

The kernel gates every MC command userspace sends through `/dev/dprc.N`
against a fixed whitelist (`drivers/bus/fsl-mc/fsl-mc-uapi.c`,
`fsl_mc_accepted_cmds[]`); a command off the list is refused `-EACCES`
before the firmware sees it. `docs/baseline/mc-ioctl-policy.md` is that
whitelist as a committed table, generated by
`models/helpers/mc-ioctl-policy.py` from the reference kernel tree
`lf-6.6.52-2.2.0-34-gf3ce2b8e0aec` (`fsl-mc-uapi.c` last changed in
`a2ad5533f862`, part of the `lf-6.6.52-2.2.0` release; the tree carries no
local fsl-mc patches) and from restool's flib headers at tag
`lf-6.6.52-2.2.0`, the release the board's v2.4 comes from. It resolves
every restool verb the adapter and the harness drive to its command ids
and verdict (task 6.5); the same generator writes the list as a Quint
module (`models/core/ioctl_policy.qnt`, a per-verb `VERB_OK` flag) the
machine checks every action against (`lastVerbs` / `IOCTL_OK`). Two things it settles for this pair: every verb the
series drives is inside the list, and the two raw probes the board program
deferred (V-DPNI-4, V-LINK-3) are outside it — their route is a kernel
patch or the VFIO transport, not the online driver (ADR-0004 §7). [read]
