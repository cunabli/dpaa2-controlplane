...
# generated by dpaa2-verify from models/board/V-DPIO-1/vdpio1.itf.json — do not edit; regenerate instead.
# operator: run as root — 1 steps issue MC commands the kernel gates on CAP_NET_ADMIN (docs/baseline/mc-ioctl-policy.md)
...
#   dpaa2-verify diff --plan V-DPIO-1.plan.json --results <dir>
set -u
...
KMSG="dpaa2-verify V-DPIO-1 pid $$"
...
# step 0: CreateObject { fam: Dpio, container: ObjRef { fam: Dprc, num: 1 } }
OBJ_dpio_0="$(run_create 0 restool --script dpio create --channel-mode=DPIO_NO_CHANNEL --num-priorities=8 --container=dprc.1)"
echo "dpio_0 ${OBJ_dpio_0}" >> "$RESULTS/created.txt"
# expect: dpio_0 present=true plugged=false
probe 0 0 restool dprc show dprc.1

# --- suite hook: models/board/V-DPIO-1/seats.sh ---
...
if grep -nE 'dpmac[.]3([^0-9]|$)|dpmac[.]17([^0-9]|$)|dpni[.]0([^0-9]|$)' "models/board/V-DPIO-1/seats.sh" | grep -v safety-self-check; then
  echo "refusing: total-deny object referenced in the hook" >&2  # safety-self-check
  exit 1
fi
. "models/board/V-DPIO-1/seats.sh"

echo "suite V-DPIO-1 complete"
