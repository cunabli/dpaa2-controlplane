...
# generated by dpaa2-verify from models/board/V-DPRC-6/vdprc6.itf.json — do not edit; regenerate instead.
...
#   dpaa2-verify diff --plan V-DPRC-6.plan.json --results <dir>
set -u
...
KMSG="dpaa2-verify V-DPRC-6 pid $$"
...
# step 3: Plug { obj: ObjRef { fam: Dpbp, num: 0 } }
run 3 restool dprc assign ${OBJ_dprc_2} --object=${OBJ_dpbp_0} --plugged=1
# expect: dpbp_0 present=true plugged=true
probe 3 0 restool dprc show ${OBJ_dprc_2}

# --- suite hook: models/board/V-DPRC-6/moves.sh ---
...
if grep -nE 'dpmac[.]3([^0-9]|$)|dpmac[.]17([^0-9]|$)|dpni[.]0([^0-9]|$)' "models/board/V-DPRC-6/moves.sh" | grep -v safety-self-check; then
  echo "refusing: total-deny object referenced in the hook" >&2  # safety-self-check
  exit 1
fi
. "models/board/V-DPRC-6/moves.sh"

echo "suite V-DPRC-6 complete"
