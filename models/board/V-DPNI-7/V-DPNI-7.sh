...
# generated by dpaa2-verify from models/board/V-DPNI-7/vdpni7.itf.json — do not edit; regenerate instead.
...
#   dpaa2-verify diff --plan V-DPNI-7.plan.json --results <dir>
set -u
...
KMSG="dpaa2-verify V-DPNI-7 pid $$"
...
OBJ_dpni_0="$(run_create 1 restool --script dpni create --options=DPNI_OPT_TX_FRM_RELEASE,DPNI_OPT_HAS_POLICING,DPNI_OPT_SHARED_CONGESTION --container=${OBJ_dprc_2})"
echo "dpni_0 ${OBJ_dpni_0}" >> "$RESULTS/created.txt"
# expect: dpni_0 present=true plugged=false
probe 1 0 restool dprc show ${OBJ_dprc_2}

# --- suite hook: models/board/V-DPNI-7/readback.sh ---
...
if grep -nE 'dpmac[.]3([^0-9]|$)|dpmac[.]17([^0-9]|$)|dpni[.]0([^0-9]|$)' "models/board/V-DPNI-7/readback.sh" | grep -v safety-self-check; then
  echo "refusing: total-deny object referenced in the hook" >&2  # safety-self-check
  exit 1
fi
. "models/board/V-DPNI-7/readback.sh"

echo "suite V-DPNI-7 complete"
