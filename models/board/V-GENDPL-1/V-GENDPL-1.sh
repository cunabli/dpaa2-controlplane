...
# generated by dpaa2-verify from models/board/V-GENDPL-1/vgendpl1.itf.json — do not edit; regenerate instead.
...
#   dpaa2-verify diff --plan V-GENDPL-1.plan.json --results <dir>
set -u
...
KMSG="dpaa2-verify V-GENDPL-1 pid $$"
...
OBJ_dpdcei_0="$(run_create 1 restool --script dpdcei create --engine=DPDCEI_ENGINE_DECOMPRESSION --priority=2 --container=${OBJ_dprc_2})"
...
OBJ_dpdmai_0="$(run_create 2 restool --script dpdmai create --priorities=2,4 --container=${OBJ_dprc_2})"
...
OBJ_dpci_0="$(run_create 3 restool --script dpci create --num-priorities=2 --container=${OBJ_dprc_2})"
echo "dpci_0 ${OBJ_dpci_0}" >> "$RESULTS/created.txt"
# expect: dpci_0 present=true plugged=false
probe 3 0 restool dprc show ${OBJ_dprc_2}

# --- suite hook: models/board/V-GENDPL-1/dpl.sh ---
...
if grep -nE 'dpmac[.]3([^0-9]|$)|dpmac[.]17([^0-9]|$)|dpni[.]0([^0-9]|$)' "models/board/V-GENDPL-1/dpl.sh" | grep -v safety-self-check; then
  echo "refusing: total-deny object referenced in the hook" >&2  # safety-self-check
  exit 1
fi
. "models/board/V-GENDPL-1/dpl.sh"

echo "suite V-GENDPL-1 complete"
