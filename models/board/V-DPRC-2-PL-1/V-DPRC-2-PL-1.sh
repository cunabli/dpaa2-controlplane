...
# generated by dpaa2-verify from models/board/V-DPRC-2-PL-1/vdprc2pl.itf.json — do not edit; regenerate instead.
...
#   dpaa2-verify diff --plan V-DPRC-2-PL-1.plan.json --results <dir>
set -u
...
KMSG="dpaa2-verify V-DPRC-2-PL-1 pid $$"
...
OBJ_dprc_2="$(run_create 0 restool --script dprc create dprc.1 --options=DPRC_CFG_OPT_SPAWN_ALLOWED,DPRC_CFG_OPT_ALLOC_ALLOWED,DPRC_CFG_OPT_OBJ_CREATE_ALLOWED,DPRC_CFG_OPT_IRQ_CFG_ALLOWED,DPRC_CFG_OPT_PL_ALLOWED)"
...
# --- suite hook: models/board/V-DPRC-2-PL-1/pl.sh ---
...
if grep -nE 'dpmac[.]3([^0-9]|$)|dpmac[.]17([^0-9]|$)|dpni[.]0([^0-9]|$)' "models/board/V-DPRC-2-PL-1/pl.sh" | grep -v safety-self-check; then
  echo "refusing: total-deny object referenced in the hook" >&2  # safety-self-check
  exit 1
fi
. "models/board/V-DPRC-2-PL-1/pl.sh"

echo "suite V-DPRC-2-PL-1 complete"
