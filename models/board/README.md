...
| V-POOL-5 | `vpool5.qnt` + `ceiling.sh` (suite hook) | authored 2026-09-20 (pool-objects task 4.1, bead dpaa2-controlplane-960.10); pending first sitting — the dpbp census/ceiling (DPBP-I7) and bpid-vs-id read-back (DPBP-I5) at ROOT scope (pool-objects design D5: the reconciler's pool is dprc.1), `--pool-record` on. The trace creates one dpbp in the root; the hook reads the census, sweeps dpbp creates in the ROOT until the No-resources refusal (predicted to the object by the `bp` free count, the refusal-is-an-answer convention), then destroys each with a settle and a residents guard (ADR-0008 §4). 4.2: this briefly draws the root buffer-pool to zero and destroys ~63 dpbps in the Linux root — run from a clean boot, read residents.txt |
| V-POOL-7 | `vpool7.qnt` + `drain.sh` (suite hook) | authored 2026-09-20 (pool-objects task 4.1, bead dpaa2-controlplane-960.10); pending first sitting — the pool free/drain path observed through the kernel face at ROOT scope, on V-LIFE-DPNI-1/V-DPNI-3's root dpni bind trace, `--pool-record` on: the bind's allocator draw (DPBP-I2 root-kernel half), the sysfs unbind's return (DPBP-I3 dirty return, DPCON-I5 free path, DPMCP-I3 observable half — RECORD-only, the free path is not restool-observable) and a re-bind proving the freed companions are re-drawable (the one judged claim). Observing, not driving (pool-objects design D3) |
| V-DPIO-1 | `vdpio1.qnt` + `seats.sh` (suite hook) | authored 2026-09-20 (pool-objects task 4.1, bead dpaa2-controlplane-960.10); pending first sitting — the dpio seat/priority surface (DPIO-I3) at ROOT scope: the boot kernel dpios' seat/priority read, a scratch DPIO_NO_CHANNEL dpio at 8 priorities (`--create-args`) with `num_priorities` read back mode-independently (the V-READBACK-1 settled half), and DPIO-I3's kernel half NAMED unreachable at root — a runtime dpio takes no fsl_mc_dpio seat (boot seats full, ADR-0008), so the DPL-child escape (bead dpaa2-controlplane-960.13) is the route and is deliberately not taken here (the loud-not-silent rule) |
| V-POOL-6 | `intent-a.toml` + `intent-b.toml` (hand-authored `dpaa2ctl` suite) | authored 2026-09-20 (pool-objects task 4.1, bead dpaa2-controlplane-960.10); pending first sitting — root-scope pool convergence through the shipped `dpaa2ctl` (task 3.4: `converge_pools` runs before the port loop): dry-run the grow, the hitless-refusal leg (a pool create is `Class::Disruptive`, so a default ensure refuses changing nothing), grow under `--allow disruptive`, idempotent re-run (zero pool actions), a foreign restool dpbp pruned as undeclared/non-DPL-born/free, and intent-b's free-only shrink of the `[extra.kernel]` surplus with the DPL-born boot pool untouched (pool-objects design D3). Operand derivation pinned by `crates/dpaa2-tools/tests/vpool6_intents.rs`; ShrinkBelowDraw stays twin-covered (nothing draws these pools under `--no-link`). 4.2: run LAST and REBOOT after — dpio seats are grow-only (pool-objects design D4), reclaimed only by the recovery reboot (ADR-0003 §7) |
