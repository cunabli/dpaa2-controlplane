...
| V-DPRC-13 | `V-DPRC-13.sh` (hand-authored) | **pending — awaiting operator window** (dprc-hardening task 2.1, bead dpaa2-controlplane-am0.2) — OI-3 / PASS3-F13-OQ: censuses the MC-global `mcp` pool free count across a baseline plus three trivial read-only `restool dprc show` spawns and a post-settle reading, to settle whether a restool spawn draws from the never-returned per-boot dpmcp budget (a leak) or returns its portal (latency only). Three samples separate a flat pool from a linear draw from a one-off. Creates and destroys nothing — no scratch child, no lock — so the ADR-0011 one-portal-per-container-cycle confound (mcp 203 → 202) cannot arise and the sitting is safe to leave standing between windows. OI-1 (the duplicate-id-under-lock ordering, PASS2-F5) rode this task but was adjudicated **off-board**: a duplicate id is not constructible through restool — no create verb pins an object id and ids mint lowest-free in one global namespace per family (ADR-0010), so the MC's duplicate check is never reached — resolved by quint directed evidence plus a note on ADR-0002 (2026-09-15) rather than a sitting |
