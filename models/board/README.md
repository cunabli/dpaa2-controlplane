...
| V-DPDBG-2 | `V-DPDBG-2.sh` (hand-authored) | **passed** 2026-09-14 (rev 1), 8/8 (dprc-encapsulation task 6.1, bead dpaa2-controlplane-cd3.14) — DPDBG-I4's sysfs face, the register's last #4-owned pointer. The created singleton's bus node was present right after create with no sync or rescan issued (root-container autorescan, the V-DPRC-4/V-DPRC-5 route), held no driver link either unplugged or plugged (modalias `fsl-mc:v00001957ddpdbg`; no driver exists for the type), and was gone in the post-destroy read-back — teardown judged on read-back because `restool dpdbg destroy` prints success unconditionally. Residents pre/post PASS, dmesg quiet, `mcp` flat 203 → 203: a root object-family create/destroy cycle moves no portal (V-POOL-4 corroborated), and with no container created this boot the ADR-0011 one-portal shape had no trigger — consistent with its first-cycle candidate reading |
...
| V-FIT-1 | `probes.json` (fit-check emitter) | **passed** 2026-09-04 (rev 2), 10/10 — the intent layer's read-only fit check (design D12): the shipped `dpaa2ctl dry-run` compiled the reference intent against the live census with zero refusals, and `status` read 6 pending, 0 drift, 0 assert-mismatch — the only divergence is the two intent ports (dpmac.7/9) Absent at the board's DPL-only boot state, pending as create/connect/bind and never drift; 25 plan-only objects reported by family, not error; the three `UnknownCeiling` warnings name exactly the families `mc.global --resources` does not list. The census corroborates ADR-0010 §4 from the MC's own account: all 97 objects unlabelled and every one present in the `generate-dpl` read-back — the whole board is DPL-born. Rev 1 (2026-09-04, FAIL 9/10) was evidentiary, not divergence: a stale pre-intent-schema `dpaa2ctl` on PATH refused the config — the sitting now resolves this checkout's own build and records its hash in the evidence (task 4.1a) |
| V-DPRC-7 | `vdprc7.qnt` (rev 2; rev 1 also had `refusals.sh`) | rev 1 **FAIL, evidentiary** 2026-09-13 (dprc-encapsulation task 5.1, bead dpaa2-controlplane-cd3.10): all 8 trace steps PASS and the hook's lock accepts PASS, but the hook's destroy-with-plugged-resident prediction was inverted by the board — `dprc destroy` of the scratch child holding a plugged owned dpbp exited 0 and cascade-destroyed the container and its resident, with no MC or restool refusal. The verdict stands as recorded; the refusal prediction was wrong, not the board, so the model's conservative destroy fence is extended (an owned resident may be plugged), not contradicted. Also observed: `mcp` 203 → 201 across the sitting with zero dpmcp created or destroyed — a datum for ADR-0011's "listing lags or uncounted reserve" question. **passed** 2026-09-13 (rev 2), 9/9, pre-vs-post census zero deltas (objects and pools): a 9-step trace (no hook) — create dprc.2/dprc.3, a dpbp in each, plug both, unplug dprc.2's, destroy dprc.2 holding its unplugged owned dpbp, then destroy dprc.3 holding its **still-plugged** owned dpbp.1 (the widened owned-plugged evict-on-destroy face, the rev-1 anchor; ADR-0007 §3). Regenerated with `--pool-record`, so it is ADR-0011's per-step pool instrument (`pool-baseline.txt` + `step-<N>-pool.txt`). The instrument answered: `mcp` flat at 201 through every step — the 203 → 201 stands as boot state from the rev-1 window, and the candidates left are the lock accepts, the refused commands, or ADR-0011's reserve/lag reading — while `bp` moved symmetrically to the object, drawn at each dpbp create and returned by both destroys, the cascade destroy of the still-plugged dpbp included. No hook: the lock/unlock accept faces are double-anchored (V-DPRC-3 rev 2, 2/2 + hook 6/6, and this suite's rev-1 hook 3/3). Refusal-shape coverage for this suite's permission matrix is already board-settled and is NOT re-probed here: 0x6 Configuration error (SPAWN absent) — V-DPRC-2-NOSPAWN-1 rev2 (2/2); 0x8 No resources (ALLOC absent) — V-DPRC-2-NOALLOC-1 rev2 (2/2); 0x4 No privilege under lock — V-DPRC-3 rev2 (2/2 + hook 6/6); plugged-move `Error::RestoolGuard` client-guard pre-check and sibling-move 0x4 No privilege — V-DPRC-6 rev1 (4/4 + hook 3/3) |
| V-DPRC-8 | `vdprc8.qnt` + `vfio.sh` (suite hook) | rev 1 **FAIL, evidentiary** 2026-09-13 (dprc-encapsulation task 5.2, bead dpaa2-controlplane-cd3.11): trace 2/2, hook 4 PASS / 1 FAIL — every acceptance face held. Override + bind of the unplugged scratch child accepted (driver symlink to vfio-fsl-mc, iommu group 18), DPRC-I7 censuses unmoved across bind and unbind, unbind restored eligibility (re-bind clean), pre-vs-post snapshots zero deltas. The FAIL is the propagation oracle judged one face early: a dpbp created while the container is bound is accepted by the MC but gets no bus device until the next bind-time container scan — the face-(j) re-bind surfaced it with the override propagated and in the container's iommu group (the sitting's dmesg carries both lines for dpbp.2). The design open question is answered: override propagation to a subsequently-added resident is real and deferred-to-scan; no restool verb triggers that scan while the container stays bound. Baseline amendment carried to task 6.1. Also observed, recorded not judged: the fresh child's bus node was already present at hook time (so neither `dprc sync` nor the bus rescan was discriminated as the surfacing route), and `mcp` stayed flat at 201 through every step (a datum for ADR-0011's open 203->201 question) |
| V-DPRC-9 | `intent-a.toml` + `intent-b.toml` (hand-authored `dpaa2ctl` suite) | **passed** 2026-09-14 (rev 1), 9/9 (dprc-encapsulation task 5.3, bead dpaa2-controlplane-cd3.12): the end-to-end product path on hardware — dry-run headlined the single `[disruptive]` CreateContainer with per-object provenance; the first ensure created dprc.2 labelled `vdprc9`; read-back matched derivation exactly (options 0x47 decoding to the DPRC-I4 default, label, root placement, zero residents); the second ensure and `status` both reported converged with zero actions (level-triggered idempotence through the shipped binary, hash in the evidence). The declared-empty prune leg classified dprc.2 `PruneCandidateFull` with matched `[Label, OptionsMask, Placement]` — the fingerprint judged on the real mask read-back, not an assumed default — and destroyed it through the reconciler's own plan (DPRC-I9 exercised under plans, not hand verbs), the predicted post-state "Destroyed, parent gains 0 residents" borne out; pre-vs-post census identical (`dprc list` and the 97-object root show), dmesg quiet. Scope note: this boot had no pre-existing child container, so the report-only fence (unlabelled DPL-born container untouched) had no live operand — 0 report-only observed; that face stays pinned by the offline bucket tests. Also observed, recorded not judged: `mcp` 203 → 202 across the sitting (the create drew a portal the immediate post-destroy pool read did not show returned) — a datum for ADR-0011's open portal-count question, carried to task 6.1 |
| V-DPRC-10 | `probes.json` (online driver, `drive --probes`) | **passed** 2026-09-14 (rev 2), 17/17 (dprc-encapsulation task 5.4, bead dpaa2-controlplane-cd3.13) — DPRC-I1 pool boundary, the online-MBT discovery debut (dprc-encapsulation design D7). Root's dpcon and dpbp pools are both locally dry at the reference boot (16 dpcons and dpbp.0 all plugged/held; both move contingencies no-ops) while the scratch sibling held two free dpcons and two free dpbps; the scratch root dpni's dpaa2-eth probe was refused by dprc.1's own allocator (`No more resources of type dpcon left`, the -ENXIO shape) and all four sibling objects read back resident and unplugged — the draw refused locally with genuine surplus one hop away, so the boundary is witnessed, not vacuous. Censuses identical, `bp` flat 63; `mcp` 203 → 202 across the sitting from a fresh boot — the second corroboration of the one-portal-per-container-cycle shape (V-DPRC-9 rev 1 saw the same; ADR-0011 datum, task 6.1 with dprc-encapsulation). Rev 1 (15/15 mechanical, same refusal line) was evidentiary only: its surplus was staged dpbp-only while the probe ran dry at dpcon — a plan staging error (triage: implementation, model and board both behaved), corrected at rev 2 with both-family surplus and per-family count/move contingencies |
...
| V-POOL-5 | `vpool5.qnt` + `ceiling.sh` (suite hook) | **passed** 2026-09-22 (rev 1), trace 1/1 (VERDICTS.json) — DPBP-I7 at ROOT scope held exactly: the census predicted refusal after 62 root dpbp creates, the refusal landed at 62 with No resources, and every unit returned (`bp` free 62→62 after the spaced destroys). Two records ride the pass. (1) DPBP-I5 is void this run: a `field` helper arity bug (leading `dpbp` arg opened as a file — fixed in this change) parsed empty ids, so the logged `bpid-equals-id=yes` is empty-vs-empty, spurious; the raw `ceiling-dpbp-info.txt` capture survives for offline judgment, rev 2 re-reads it. (2) The ADR-0008 §4 destroy-rescan race fired mid-sweep despite the §6 1s spacing: dmesg carries the §7 signature (`dpni.0 eth0: Link is Down` at 1317.3s, boot dpcon.0 re-added to its iommu group 0.6s later) and the management interface dropped (operator observed the outage), yet `residents post` PASSed — the remove/re-add arm healed the driver links before the hook read them, so the arrival-time residents guard missed a transient casualty and dmesg is the only evidence. ~62 spaced root destroys is an order of magnitude past the five-destroy re-sitting that validated §6's spacing, and the hook-side root destroys conflict with §7's placement rule — carried as a 4.2 finding. **passed** 2026-09-22 (rev 2): record (1) settled — the fixed `field` helper read the ids for real and DPBP-I5 held (dpbp id=1 equals buffer pool id 1); DPBP-I7 re-held at 62; and the §4 race of record (2) fired AGAIN under the same §6 spacing and SELF-HEALED (dmesg: `dpni.0` link Down at 4376.5s, boot dpcon.0 re-added +0.6s, link back Up +37s) — a twice-observed transient, the arrival-time residents guard passing both times |
| V-POOL-7 | `vpool7.qnt` + `drain.sh` (suite hook) | **passed** 2026-09-22 (rev 1), 9/9 trace + hook clean (VERDICTS.json) — the full draw→return→redraw cycle at ROOT scope (DPBP-I2/I3's board face): dpni.1 kernel-bound (eth1, `fsl_dpaa2_eth`) with companions dpmcp.53/dpbp.1/dpcon.16 drawn by `fsl_mc_allocator`; the sysfs unbind released them (netdev gone); the re-bind re-drew the freed companions (eth1 back). Observing, not driving (pool-objects design D3) |
| V-DPIO-1 | `vdpio1.qnt` + `seats.sh` (suite hook) | **passed** 2026-09-22 (rev 1), 1/1 trace + hook (VERDICTS.json), residents pre/post PASS — the dpio seat/priority surface (DPIO-I3) at ROOT scope: boot dpio.0/dpio.15 read DPIO_LOCAL_CHANNEL with 0x8 priorities, and the scratch runtime dpio.16 reads DPIO_NO_CHANNEL with 8 priorities mode-independently (the V-READBACK-1 settled half). DPIO-I3's kernel half is formally recorded UNREACHABLE at root — the runtime dpio holds no `fsl_mc_dpio` seat (boot seats full, ADR-0008), so the DPL-child escape (bead dpaa2-controlplane-960.13) is the named route for the 4.4 gate, deliberately not taken here (the loud-not-silent rule) |
| V-POOL-6 | `intent-a.toml` + `intent-b.toml` (hand-authored `dpaa2ctl` suite) | **FAILED, harness-side; the model did not diverge** 2026-09-22 (rev 1 + rev 2) — both runs exit-for-exit identical across a reboot (deterministic). Every ensure leg (steps 3/5/8) refused: `Dpmcp requirement 19 is below the drawn count 52` — the root holds ~52 plugged DPL-born dpmcp (boot pool + kernel companions) and the Rust census (`census_of`) folded all plugged rows into the guard's drawn count, while the model's `drawnManaged` counts managed only. The refusal preempted the grow, so the foreign dpbp (dpbp.1) was never pruned (step 9's FAIL is downstream). The bead's open census-netting judgment is answered: the sizing census must net DPL-born out. (CORRECTED 2026-09-24: the prune path did NOT discriminate — both rev 1 and rev 3 step-7 captures read `prune (0 candidate(s))` with the out-of-band dpbp standing; an empty-label create judges DPL-born in the membership judge and escapes prune, one of the two disagreeing label judges pool-objects design D10 reconciles.) Rust-twin fix in the paired change; model and frozen traces unchanged. Steps 0/1/2/7 passed (the dry-runs headline the root pool convergence block; the hitless ensure refuses the Disruptive grow as designed). Rev 3 (fixed census binary, 2026-09-22) **half-advanced**: the dpmcp netting refusal is gone (the DPL-born census netting held on board), step 3 then died on the shim's `disconnect --endpoint1=` flag (the baseline speaks `--endpoint=`; fixed with bead 960.18, commit f06fbb3), and steps 5/8 refused `requirement 19 below drawn 20` — the port chain's own companion dpmcp.72 (label `kern0`) counted into the pool census, the double-feed that decided single provider (pool-objects design D9, beads dpaa2-controlplane-960.17–.20); **rev 4 pending on the single-provider + custody-law binary** (pool-objects tasks 3.9–3.14: the 2026-09-24 audit found the shrink and prune legs as written assert laws the shipped census cannot execute — plugged counts as drawn, so the intent-a→b shrink would refuse, and the empty-label foreign escapes prune; the suite walk is revised at task 3.14) |
