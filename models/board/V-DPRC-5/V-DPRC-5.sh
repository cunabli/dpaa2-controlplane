...
# generated by dpaa2-verify from models/board/V-DPRC-5/vdprc5.itf.json — do not edit; regenerate instead.
...
#   dpaa2-verify diff --plan V-DPRC-5.plan.json --results <dir>
set -u
...
KMSG="dpaa2-verify V-DPRC-5 pid $$"
...
teardown() {
  [ -n "${OBJ_dpci_1:-}" ] && [ -e "/sys/bus/fsl-mc/devices/${OBJ_dpci_1}/driver" ] && echo "${OBJ_dpci_1}" > "/sys/bus/fsl-mc/devices/${OBJ_dpci_1}/driver/unbind" 2>>"$RESULTS/teardown.log" || true
  [ -n "${OBJ_dpci_1:-}" ] && restool dprc assign dprc.1 --object="${OBJ_dpci_1}" --plugged=0 2>>"$RESULTS/teardown.log" || true
...
# step 2: CreateObject { fam: Dpci, container: ObjRef { fam: Dprc, num: 1 } }
OBJ_dpci_1="$(run_create 2 restool --script dpci create --container=dprc.1)"
echo "dpci_1 ${OBJ_dpci_1}" >> "$RESULTS/created.txt"
# expect: dpci_1 present=true plugged=false
probe 2 0 restool dprc show dprc.1

# --- suite hook: models/board/V-DPRC-5/visibility.sh ---
...
if grep -nE 'dpmac[.]3([^0-9]|$)|dpmac[.]17([^0-9]|$)|dpni[.]0([^0-9]|$)' "models/board/V-DPRC-5/visibility.sh" | grep -v safety-self-check; then
  echo "refusing: total-deny object referenced in the hook" >&2  # safety-self-check
  exit 1
fi
. "models/board/V-DPRC-5/visibility.sh"

echo "suite V-DPRC-5 complete"
