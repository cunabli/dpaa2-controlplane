...
# generated by dpaa2-verify from models/board/V-DPNI-5/vdpni5.itf.json — do not edit; regenerate instead.
# operator: run as root — 2 steps issue MC commands the kernel gates on CAP_NET_ADMIN (docs/baseline/mc-ioctl-policy.md)
...
#   dpaa2-verify diff --plan V-DPNI-5.plan.json --results <dir>
set -u
...
KMSG="dpaa2-verify V-DPNI-5 pid $$"
...
OBJ_dpni_0="$(run_create 1 restool --script dpni create --options=DPNI_OPT_SINGLE_SENDER,DPNI_OPT_CUSTOM_CG,DPNI_OPT_HAS_KEY_MASKING,DPNI_OPT_HAS_OPR,DPNI_OPT_OPR_PER_TC,0x80000000 --num-queues=16 --num-tcs=16 --num-cgs=24 --vlan-entries=16 --qos-entries=64 --fs-entries=1 --container=${OBJ_dprc_2})"
echo "dpni_0 ${OBJ_dpni_0}" >> "$RESULTS/created.txt"
# expect: dpni_0 present=true plugged=false
probe 1 0 restool dprc show ${OBJ_dprc_2}

# --- suite hook: models/board/V-DPNI-5/readback.sh ---
...
if grep -nE 'dpmac[.]3([^0-9]|$)|dpmac[.]17([^0-9]|$)|dpni[.]0([^0-9]|$)' "models/board/V-DPNI-5/readback.sh" | grep -v safety-self-check; then
  echo "refusing: total-deny object referenced in the hook" >&2  # safety-self-check
  exit 1
fi
. "models/board/V-DPNI-5/readback.sh"

echo "suite V-DPNI-5 complete"
