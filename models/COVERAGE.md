...
| DPBP-I5 | modeled | `core/invariants.qnt` `LAW4_twoIdSpaces` + `main.qnt` `DPBP_I5Test` | apalache | open: V-READBACK-1 (2026-08-25) read a runtime dpbp back with bpid equal to its object id, so divergence stays unobserved on this board — the law is kept as a prohibition, nothing relies on equality (dpbp.md unknown 2). V-POOL-5 rev 1 (2026-09-22) re-read the same face but the run is void: a `field` helper arity bug parsed empty ids, so its `bpid-equals-id=yes` is empty-vs-empty and spurious; the helper is fixed and the raw `ceiling-dpbp-info.txt` capture survives for offline judgment, so rev 2 re-reads it → `pool-objects` (#6) |
