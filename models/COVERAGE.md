...
|-----------|-------------|----------------------------------------------|---------|--------------|
| DPRC-I1 | modeled | `core/invariants.qnt` `DPRC_I1` + `families/dprc.qnt` `dprc_lifecycle` `DPRC_I1Test` (eviction boundary: a created resident is released with the container, never crossing into the parent) (also DPMCP-I4's placement face) | apalache | — |
...
| DPRC-I11 | modeled | `main.qnt` `DPRC_I11Test` + `DPRC_I11SpawnTest`/`DPRC_I11UnlockTest` + `families/dprc.qnt` `dprc_lifecycle` `lockStripsCreateTest`/`lockRefusesPlugTest`/`unlockRestoresTest` (the restool-reachable lock-strip remainder as lifecycle guards) and `labelUnderLockTest` | simulate | open: rev 1 (V-DPRC-3, 2026-08-29, 2/2 + hook 6/7) observed a lock set from the root refusing assign in the child (No privilege, dpbp reads back unplugged), leaving reads working, and lifted by the root; its one hook FAIL was a wrong prediction — `set-label` is *not* stripped — so the corrected hook settles the row at rev 2. The label-under-lock open question resolved to repairable (set-label accepted under lock, V-DPRC-3), encoded in `dprc_lifecycle` `setLabelAt`/`labelUnderLockTest`. The child-portal unlock face stays unreachable through restool (it always opens the parent portal) → `mc-portal-backend` (#10) |
